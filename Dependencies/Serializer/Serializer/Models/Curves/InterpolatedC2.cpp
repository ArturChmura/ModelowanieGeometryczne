...
namespace MG1
{
	...
	InterpolatedC0::InterpolatedC0(Bezier&& other)
		: Bezier(std::move(other))
	{   }

	void to_json(nlohmann::ordered_json& j, const InterpolatedC0& p)
	{
		auto points = nlohmann::json::array();
		...
		j = nlohmann::ordered_json();
		j["objectType"] = "interpolatedC0";
		j["id"] = p.m_id;
		j["name"] = p.name;
		j["controlPoints"] = points;
	}

	void from_json(const nlohmann::ordered_json& j, InterpolatedC0& p)
	{
		if (j.contains("name"))
		{
			p.name = j["name"];
